...
int APIENTRY WinMain(HINSTANCE hInstance,HINSTANCE hPrevInstance,LPSTR lpCmdLine,int nCmdShow) // OK
{
	...
	{
		MessageBox(0,"Could not open Jogar.exe (#1)","Error",MB_OK | MB_ICONERROR);
		return 0;
	}
	...
	wsprintf(CommandLine,"jogar.exe -test -k%d",GetTickCount());

	if(CreateProcess(0,CommandLine,0,0,0,NORMAL_PRIORITY_CLASS | CREATE_SUSPENDED,0,0,&si,&pi) == 0)
	{
		MessageBox(0,"Could not open jogar.exe (#2)","Error",MB_OK | MB_ICONERROR);
		return 0;
	}
	...
	if(ProcessHandle == 0)
	{
		MessageBox(0,"Could not open jogar.exe (#3)","Error",MB_OK | MB_ICONERROR);
		return 0;
	}
	...
	if(ThreadHandle == 0)
	{
		...
		MessageBox(0,"Could not open jogar.exe (#4)","Error",MB_OK | MB_ICONERROR);
		return 0;
	}
	...
	if(context.Eip != MAIN_EIP)
	{
		...
		//CloseHandle(ProcessHandle);
		//MessageBox(0,"Could not open jogar.exe (#5)","Error",MB_OK | MB_ICONERROR);
		return 0;
	}
